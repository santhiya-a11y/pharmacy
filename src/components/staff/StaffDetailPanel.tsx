...
const StaffDetailPanel = ({ member, onClose }: Props) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/40 backdrop-blur-sm" onClick={onClose}>
      <div
        className="relative w-full max-w-2xl max-h-[85vh] rounded-2xl border border-border bg-card shadow-2xl animate-in fade-in zoom-in-95 duration-200 flex flex-col"
            ...
            <>
                ...
                <div className="rounded-xl border border-border p-4 space-y-3">
                  ...
                  <div className="space-y-2.5">
                    ...
                    <div className="flex items-center gap-2.5 text-sm">
                      ...
                      <span className="text-card-foreground truncate">{member.email}</span>
                    </div>
                  </div>
                </div>
                <div className="rounded-xl border border-border p-4 space-y-3">
                  ...
                  <div className="space-y-2.5">
                    <div className="flex items-center gap-2.5 text-sm">
                      ...
                      <span className="text-card-foreground">{member.role}</span>
                    </div>
                    <div className="flex items-center gap-2.5 text-sm">
                      <Clock className="h-4 w-4 text-muted-foreground" />
                      <span className="text-card-foreground">{member.shift} Shift</span>
                    </div>
                    <div className="flex items-center gap-2.5 text-sm">
                      <Calendar className="h-4 w-4 text-muted-foreground" />
                      <span className="text-card-foreground">Since {member.joinDate}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-border p-4">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex h-9 w-9 items-center justify-center rounded-full bg-muted text-muted-foreground text-xs font-bold">EC</div>
                  <div>
                    <p className="text-sm font-medium text-card-foreground">Family Member</p>
                    <p className="text-xs text-muted-foreground">+91 99999 00000</p>
                  </div>
                </div>
              </div>
            </>
                ...
                {documents.map(doc => (
                  <div key={doc.id} className="flex items-center gap-3 rounded-xl border border-border p-3.5 hover:shadow-sm transition-shadow group">
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button className="rounded-md p-1.5 hover:bg-secondary transition-colors" title="View">
                        <Eye className="h-3.5 w-3.5 text-muted-foreground" />
                      </button>
                      <button className="rounded-md p-1.5 hover:bg-secondary transition-colors" title="Edit">
                        <Edit2 className="h-3.5 w-3.5 text-muted-foreground" />
                      </button>
                      <button onClick={() => handleDeleteDoc(doc.id)} className="rounded-md p-1.5 hover:bg-destructive/10 transition-colors" title="Remove">
                    ...
                    {attendanceData.map((row, i) => (
                      <tr key={i} className="border-b border-border last:border-0">
                        ...
                        <td className="px-4 py-2.5">
                          <Badge className={`text-[10px] capitalize ${statusColors[row.status] || "bg-muted text-muted-foreground"} border-0`}>
                            {row.status}
                          </Badge>
                        </td>
